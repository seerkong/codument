...
export type TrackAuthorityFormat = 'xnl' | 'xml';

export interface TrackAuthority {
  format: TrackAuthorityFormat;
  fileName: 'track.xnl' | 'track.xml';
  file: string;
}
...
export function resolveTrackAuthority(trackDir: string): TrackAuthority | undefined {
  const xnl = path.join(trackDir, 'track.xnl');
  const xml = path.join(trackDir, 'track.xml');
  const hasXnl = fs.existsSync(xnl);
  const hasXml = fs.existsSync(xml);
  if (hasXnl && hasXml) {
    throw new Error(`Track directory has multiple Track authority files: ${xnl} and ${xml}`);
  }
  if (hasXnl) return { format: 'xnl', fileName: 'track.xnl', file: xnl };
  if (hasXml) return { format: 'xml', fileName: 'track.xml', file: xml };
  return undefined;
}

export function parseTrackResource(file: string): SpecXmlNode {
  return parseTrackResourceContent(fs.readFileSync(file, 'utf8'), file);
}

export function parseTrackResourceContent(content: string, file = 'track.xnl'): SpecXmlNode {
  ...
  if (parsed.nodes.length !== 1 || !isDataElement(parsed.nodes[0])) {
    throw new Error('Track XNL must contain exactly one data-element root.');
  }
  const root = fromXnlElement(parsed.nodes[0]);
  if (root.tag !== 'Track') throw new Error(`Track XNL root must be <Track>, received <${root.tag}>`);
  ...
  for (const [key, value] of Object.entries(parsed.nodes[0].attributes ?? {})) {
    ...
    if (ROOT_METADATA_TAGS.has(field)) metadata.children.push(textNode(field, requireScalar(value, `Track.${key}`)));
  }
...
export function convertLegacyTrackXml(content: string, targetApiVersion?: string): string {
  const root = parseSpecXmlContent(content);
  if (root.tag !== 'Track') throw new Error(`Legacy Track root must be <Track>, received <${root.tag}>`);
...
function fromXnlElement(node: DataElementNode | TextElementNode): SpecXmlNode {
  ...
  for (const [key, value] of Object.entries(node.attributes ?? {})) {
    if (node.tag === 'Track' && ROOT_METADATA_TAGS.has(snakeToPascal(key))) continue;
...
function requireScalar(value: XnlNode, location: string): string {
  ...
  if (normalized === undefined) throw new Error(`${location} must be a scalar Track property.`);
  return normalized;
}
