...
describe("generateCodexCommands", () => {
  it("installs generated codument skills into the Codex skill directory", async () => {
    ...
    expect(fs.readFileSync(path.join(skillRoot, "SKILL.md"), "utf-8")).toContain("| `codument:init` / `codument-init` |");
    expect(fs.readFileSync(path.join(skillRoot, "SKILL.md"), "utf-8")).toContain("| `codument:gap-loop` / `codument-gap-loop` |");
    ...
    expect(gapLoopRef).toContain("prefer model `gpt-5.5` or higher with `high` reasoning");
    ...
    expect(fs.readFileSync(path.join(skillRoot, "shared", "target-capabilities.md"), "utf-8")).toContain("gpt-5.5");
