...
3. **等待用户选择：** 根据选择调整起始点（使用 **Protocol: ask-single-question-closed**）
...
3. **对于每个任务：**
   ...
   b. **更新状态：** 将 plan.xml 中任务状态更新为 `IN_PROGRESS`

   c. **遵循工作流：** 严格按 workflow.md 定义的方法论执行
      - 如果是 TDD 流程：编写测试 → 实现 → 重构
      - 如果有子任务：按顺序完成所有子任务

   d. **验证验收标准：** 逐一检查 `<acceptance_criteria>` 中的每个标准
