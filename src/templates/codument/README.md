...
Codument 的核心状态文件按生命周期放在 `tracks/pending/<id>/`、`tracks/active/<id>/` 和 `tracks/archived/YYYY-MM/<timestamp>-<id>/`。其中的 `track.xnl` 把一次变更拆成三条正交轴：

| 轴 | 位置 | 作用 |
|---|---|---|
| 结构 | `<TaskSpace>` | 记录任务树、阶段、任务状态与验收信息 |
| 调度 | `<Schedule>` | 记录顺序 / DAG 依赖与并行约束 |
| 行为 | `<Hooks>` | 记录方向审查、gap-loop、人工确认、制品同步等生命周期操作 |
...
│   ├── operations/              codument-* skill 的权威 operation body
│   ├── commands/                复杂 CLI 子命令的使用说明
│   ├── protocols/               questioning、validation 等共享协议
│   ├── methods/                 TDD、DAG、workflow 等共享方法
│   ├── kinds/                   Halfcode KindDefinition authority
│   └── compat/                  历史路径、名称和格式迁移映射
├── manifest.xnl                 Halfcode ResourcePackage catalog
...
├── decisions/                   按业务 owner/topic 组织的 durable Decision forest
├── modeling/                    领域与结构知识 XNL registry（有内容时创建）
├── engineering/                 长期工程知识 XNL registry（有内容时创建）
└── tracks/                      运行期变更追踪：pending / active / archived
```

## 当前标准

1. `track.xnl` 是 track 状态真源；任务状态在 XNL 内维护，不再依赖 `state.json`。
2. 第一层 `TaskGroup` 表示 phase；其下可继续嵌套 `TaskGroup` 或 `Task`。
3. 调度按层声明：需要并行的层写 `{ child_mode = "dag" }`，依赖写入 `<Schedule []>/<Dag>`。
4. `summary` 不手维护；统计由工具遍历 TaskSpace 派生。
5. 变更输入输出使用目录物料：`behavior_deltas/`、`docs/`、`codument/behaviors/` 等。
6. 能力开关使用 `config/attractor-profiles.xnl` 的 `<Profile #id { enabled = true|false }>`；不再使用 `config/feature.json`。
...
9. 跨多个 track 的长期路线放在 `missions/{pending,active,archived}/<id>/mission.xnl`；单次变更以其生命周期目录中的 `track.xnl` 为状态真源。
10. `std/operations/` 是各 `codument-*` skill 的权威提示词 body；agent skill 目录只安装薄壳入口。
11. `manifest.xnl` 只登记 Halfcode loader 当前可单层发现的正式 Kind；递归 owner-local resource 由对应 Codument CLI 加载和校验。

## 常用入口

- `std/AGENTS.md`：给 AI 助手的总入口。
- `std/spec/track-xnl-spec.md`：`track.xnl` 格式。
