...
直接读取与讨论目标相关的项目约束和现有资料；仅当 `action-hooks.xml` 显式为 `discuss:before` 配置 `<cdt:AttractorCheck>` 时才执行 fresh check。
...
使用 `codument/std/protocols/questioning.md` 的 severity：
...
- `codument/std/actions/impl-quick.md`
