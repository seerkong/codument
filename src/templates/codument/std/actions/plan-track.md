...
开始时按 `codument/std/protocols/decision-tree.md` 解析 severity 和当前决策前沿。`auto` 模式下不提问、不等待确认，改为写入假设并选择保守默认；其他模式才按 **ask-single-question-free** / **ask-multi-question-free** 提问。
...
遵循 `std/protocols/decision-tree.md`：未指定 severity 时使用 `light`，先用本地证据消除问题，再处理当前阻塞 frontier。`auto` 不提问，默认 `CommitMode=manual`，也不默认挂 `HumanConfirm`、`AttractorCheck` 或 `GapLoop`。
...
5. **按 severity 处理确认**：`auto` 模式不提问，直接继续，并把关键假设写入 `analysis/decision-tree.xnl` / `decisions.xnl`；其他模式询问："我已起草了行为规范。请审查：文件路径在 `codument/tracks/pending/<track_id>/behavior_deltas/<capability>/delta.xml`。这是否准确捕获了需求？请建议更改或确认。" 等待并修改至确认。
...
- **按 severity 处理确认**：`auto` 模式不提问，直接继续，并把未确认假设保留在提案 / decision-tree；其他模式询问："我已起草了变更提案。请审查：`codument/tracks/pending/<track_id>/proposal.md`。此提案是否正确？请建议更改或确认。" 等待并修改至确认。
...
写入 `codument/tracks/pending/<track_id>/design.md`，**按 severity 处理确认**：`auto` 模式不提问，直接继续；其他模式询问："我已起草了方案设计。请审查：`codument/tracks/pending/<track_id>/design.md`。此方案设计是否正确？请建议更改或确认。" 等待并修改至确认。
...
等待反馈、修改 `track.xml` 至确认，然后据选择写入。若用户没有明确要求且 track 不属于架构、安全或数据一致性高风险，**不挂 AttractorCheck**。高风险 track 默认只在最后一个第一层 `TaskGroup` 挂一个 `phase:after` 的 `<cdt:AttractorCheck use="coding"/>`。**校验模式塌缩到 track.xml 节点**：旧 `validation_mode`/`validation_granularity` = "在终态 phase（或每个 phase）挂哪个 typed check"——`final_phase` = 仅最后一个第一层 `TaskGroup` 挂；`every_phase` = 每个都挂。这些 typed check **配置直接写在节点上，无独立定义文件**（见 track-xml-spec §6）：
...
- `codument/std/protocols/questioning.md#ask-single-question-free` / `#ask-multi-question-free`
