...
按其中的 Markdown 说明 + `--` 流程标记块执行（解析来源=track output MaterialBundle / 目标根 → fresh 子代理生成 → 按 policy 写）；docs 路由/质量规则见 `@/codument/std/sop/artifact-sync.md`（body 引用）。

- **前置**：项目已 `codument-init`，对应 profile（如 `docs`）已 enabled，且 hook 显式配置或用户手动调用。
