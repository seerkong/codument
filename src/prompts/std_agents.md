...
仅当 `codument/config/feature.json` 中 `knowledgeSync.enabled=true` 时，spec XML delta 可在相关 `<requirement>`、`<suite>` 或 `<case>` 内添加：

```xml
<knowledge-hint target="main-docs" href="knowledge://main-docs/<plane>/<topic>" strength="hint" />
```

规则：
- `knowledge-hint` 是 docs/knowledge sync 的弱提示，不是强外键。
- 链接失效不应默认阻断 archive。
- `knowledgeSync.enabled=false` 或配置缺失时，不生成 `knowledge-hint`，也不生成 docs 联动信息。
