...
function detectTrack(workspace: string, explicit?: string): TrackLocation | undefined {
  for (const stage of ['active', 'pending'] as const) {
    const root = path.join(workspace, 'codument', 'tracks', stage);
    if (!fs.existsSync(root)) continue;
    const candidates = explicit ? [explicit] : fs.readdirSync(root).sort();
    for (const id of candidates) {
      const dir = path.join(root, id);
      if (fs.existsSync(path.join(dir, 'track.xnl')) || fs.existsSync(path.join(dir, 'track.xml'))) {
        return { id, stage, dir };
      }
    }
  }
...
function runCodument(cwd: string, name: string, cli: string, args: string[]): CheckResult {
  return cli.endsWith('.ts')
    ? run(cwd, name, 'bun', ['run', cli, ...args])
    : run(cwd, name, cli, args);
}
...
const trackLocation = detectTrack(workspace, typeof options.track === 'string' ? options.track : undefined);
const track = trackLocation?.id;
...
if (track) {
  checks.push(runCodument(workspace, 'codument validate', cli, ['validate', track, '--strict']));
  checks.push(runCodument(workspace, 'modeling validate', cli, ['modeling', 'validate', '--deltas', track]));
  checks.push(runCodument(workspace, 'engineering validate', cli, ['engineering', 'validate', '--deltas', track]));
}
...
const modelingFiles = trackLocation ? listFiles(path.join(trackLocation.dir, 'modeling_deltas'), (rel) => rel.endsWith('.xnl')) : [];
const engineeringFiles = trackLocation ? listFiles(path.join(trackLocation.dir, 'engineering_deltas'), (rel) => rel.endsWith('.xnl')) : [];
...
const dimensions = [
  ...
  dim('Codument alignment', 20, codumentScore, [
    trackLocation ? `track ${trackLocation.stage}/${trackLocation.id}` : 'no pending or active track found',
