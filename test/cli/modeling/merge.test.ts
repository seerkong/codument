...
describe('modeling 3-way merge', () => {
  it('takes the changed side when only one side changed', () => {
    const base = nodes(
      `<object #r.x { kind = "entity" fact_grade = "surface_view" single_writer = "r.s" } [ <types ?t>v1</?t> ]>`,
    );
    const ours = base;
    const theirs = nodes(
      `<object #r.x { kind = "entity" fact_grade = "surface_view" single_writer = "r.s" } [ <types ?t>v2</?t> ]>`,
    );
  ...
  it('auto-merges disjoint changes (different attribute keys)', () => {
  ...
  it('policy override resolves same-field to theirs', () => {
    ...
    expect(conflicts).toEqual([]);
    expect((merged.get('r.x') as any).attributes.a).toBe('3');
  });
